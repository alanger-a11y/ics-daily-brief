...
Runs on a cron schedule via Render. Pipeline:
  ...
  4. Send formatted email via Gmail SMTP
"""
...
GMAIL_APP_PASSWORD = os.environ["GMAIL_APP_PASSWORD"] # 16-char app password from Google
RECIPIENT_EMAIL   = os.environ["RECIPIENT_EMAIL"]    # who receives the brief (can be same as GMAIL_ADDRESS)
...
def send_email(brief):
    ...
    html_body = f"""<html><body style="font-family:Georgia,serif;max-width:680px;margin:40px auto;
